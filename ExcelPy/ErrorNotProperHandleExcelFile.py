import requests
import json
import pandas as pd
import os
from dotenv import load_dotenv

load_dotenv()
API_KEY = os.getenv("OPENWEATHER_APIKEY")
lat = os.getenv("LATTITUDE")
lon = os.getenv("LONGITUDE")
print(lat, lon, API_KEY)

CITY = "London"
URL = f"https://api.openweathermap.org/data/2.5/weather?q={CITY}&appid={API_KEY}"
URL1 = f"https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={API_KEY}"


def get_weather_data(url):
    try:
        response = requests.get(url)
        data = response.json()
        print(data)
        temperature = data['main']['temp']
        feels_like = data['main']['feels_like']
        humidity = data['main']['humidity']
        weather_description = data['weather'][0]['description']
        wind_speed = data['wind']['speed']

        # Creating the data dictionary
        dataframe = {
            "Temperature": [f"{temperature} K"],
            "Feels Like": [f"{feels_like} K"],
            "Humidity": [f"{humidity} %"],
            "Weather": [weather_description],
            "Wind Speed": [f"{wind_speed} m/s"],
            "Pressure": [f"{data['main']['pressure']} hPa"],
            "Visibility": [f"{data['visibility']} m"],
            "Cloudiness": [f"{data['clouds']['all']} %"],
            "Rain": [f"{data.get('rain', {}).get('1h', '0')} mm"],
            "Country": [data['sys']['country']]
        }

        # Convert the dictionary to a DataFrame
        df = pd.DataFrame(dataframe)

        # Save or append the DataFrame to the Excel file
        excel_file_name = 'weather1.xlsx'
        if not os.path.isfile(excel_file_name):
            # If the file doesn't exist, create it
            df.to_excel(excel_file_name, index=False)
        else:
            # If the file exists, append the new data
            with pd.ExcelWriter(excel_file_name, mode='a', if_sheet_exists='overlay', engine='openpyxl') as writer:
                df.to_excel(writer, index=False, header=False, startrow=writer.sheets['Sheet1'].max_row)

        return data

    except Exception as e:
        print(e)

    finally:
        print("Good work")


def new_data():
    # Creating a new set of data to append
    data = {
        "Temperature": ["306.15 K"],
        "Feels Like": ["312.3 K"],
        "Humidity": ["70 %"],
        "Weather": ["clear sky"],
        "Wind Speed": ["2.00 m/s"],
        "Pressure": ["1005 hPa"],
        "Visibility": ["11000 m"],
        "Cloudiness": ["0 %"],
        "Rain": ["0 mm"],
        "Country": ["IN"]
    }

    # Convert the dictionary to a DataFrame
    df = pd.DataFrame(data)
    excel_file_name = 'weather1.xlsx'

    with pd.ExcelWriter(excel_file_name, mode='a', if_sheet_exists='overlay', engine='openpyxl') as writer:
        # Calculate the startrow to append new data
        start_row = writer.sheets['Sheet1'].max_row
        lenth = pd.read_excel("weather1.xlsx",sheet_name='Sheet1')
        print('lenth',len(lenth))
        print('start row',start_row)
        df.to_excel(writer, index=False, header=False, startrow=start_row)


def main():
    get_weather_data(URL1)
    new_data()


if __name__ == "__main__":
    main()
